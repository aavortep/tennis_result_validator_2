...
def tournament_list(request):
    ...
    return render(
        request, "tournaments/tournament_list.html", {"tournaments": tournaments}
    )
...
def tournament_detail(request, pk):
    ...
    available_players = User.objects.filter(role=Role.PLAYER).exclude(
        id__in=tournament.players.values_list("id", flat=True)
    )

    available_referees = User.objects.filter(role=Role.REFEREE).exclude(
        id__in=tournament.referees.values_list("id", flat=True)
    )
...
def tournament_edit(request, pk):
    ...
    return render(
        request, "tournaments/tournament_form.html", {"tournament": tournament}
    )
