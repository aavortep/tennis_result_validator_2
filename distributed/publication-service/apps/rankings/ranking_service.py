...
class RankingService:
    ...
    def update_ranking_after_match(match):
        ...
        score = ResultsServiceClient.get_scores_by_match(match.id)[0]
    ...
    def recalculate_positions(tournament):
        rankings = Ranking.objects.filter(tournament=tournament.id).order_by(
    ...
    def finalize_tournament_rankings(tournament):
        final_match = TournamentsServiceClient.get_tournament_matches(tournament.id)[0]

        if final_match and final_match.winner:
            winner_ranking = Ranking.objects.filter(
                tournament=tournament.id, player=final_match.winner
            ).first()
        ...
        for ranking in Ranking.objects.filter(tournament=tournament.id):
    ...
    def initialize_tournament_rankings(tournament):
        for player in tournament.get_player_ids():
            Ranking.objects.get_or_create(player=player, tournament=tournament.id)
