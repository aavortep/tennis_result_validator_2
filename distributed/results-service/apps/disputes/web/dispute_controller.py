...
from ..internal.dispute_serializers import (
    ...
    def post(self, request):
        ...
        try:
            dispute = DisputeService.create_dispute(
                serializer.validated_data["match"],
...
class DisputeListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_player:
            ...
            queryset = queryset.filter(Q(match__player1=user.id) | Q(match__player2=user.id))

        elif user.is_referee:
            queryset = queryset.filter(match__referee=user.id)
